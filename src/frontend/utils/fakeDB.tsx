export const Orquestas = [
  {
    name: "Orquesta 1",
    description:
      "Lorem ipsum dolor sit amet consectetur, adipisicing elit. Fuga adipisci qui, quae numquam amet voluptate aspernatur vel libero! Eius, velit.",
    logo: "https://i.pinimg.com/originals/17/be/10/17be10c47b8cdaa77f671be0abd95f69.png",
    cover:
      "https://cdn.pixabay.com/photo/2016/11/19/09/57/violins-1838390_960_720.jpg",
    sponsors: ["Patrocinador 1", "Patrocinador 2", "Patrocinador 3"],
    ubication: "La Patagonia, Argentina",
    orchestra_type: "Sinfónica",
    donations_acount: "Terrell.Emmerich98@hotmail.com",
    phone: "535-808-3320",
  },
  {
    name: "Orquesta 2",
    description:
      "Nesciunt laboriosam vitae aut beatae porro consequatur qui vitae quos. Explicabo reprehenderit aut qui aliquid. Ratione asperiores laudantium quod ea. Ab et quis a tempore ut sed. In sint non rem quae harum est at et.",
    logo: "https://static8.depositphotos.com/1378583/986/v/600/depositphotos_9866608-stock-illustration-orchestra-logo.jpg",
    cover:
      "https://cdn.pixabay.com/photo/2018/01/18/12/39/music-3090204_960_720.jpg",
    sponsors: ["Patrocinador 1", "Patrocinador 2", "Patrocinador 3"],
    ubication: "West Virginia, Oman",
    orchestra_type: "Filarmónica",
    donations_acount: "Paxton10@yahoo.com",
    phone: "870-268-9274",
  },
  {
    name: "Orquesta 3",
    description:
      "Eius nisi repellendus. Fugiat aut similique et fuga cumque qui exercitationem nesciunt. Numquam minima aperiam at harum id quis omnis.",
    logo: "https://img.freepik.com/vector-premium/logotipo-instrumento-musical-violin-piano-grupo-orquesta_468322-227.jpg",
    cover:
      "https://cdn.pixabay.com/photo/2020/12/09/18/42/violin-5818267_960_720.jpg",
    sponsors: ["Patrocinador 1", "Patrocinador 2", "Patrocinador 3"],
    ubication: "Maryland, Haiti",
    orchestra_type: "Orquesta de Cámara",
    donations_acount: "Jamil_Roob88@yahoo.com",
    phone: "717-344-3080",
  },
  {
    name: "Orquesta 4",
    description:
      "Error nam consequatur pariatur similique temporibus. Atque provident corporis laborum in voluptatem et. Dolores quae perspiciatis illo. Corporis voluptatem placeat. Occaecati optio perferendis deserunt laborum rerum. Accusamus quo et sit praesentium accusamus ut illo sint qui.",
    logo: "https://pbs.twimg.com/profile_images/692948683344707585/kAF0LdgQ_400x400.jpg",
    cover:
      "https://cdn.pixabay.com/photo/2020/05/11/09/03/conductor-5157153_960_720.jpg",
    sponsors: ["Patrocinador 1", "Patrocinador 2", "Patrocinador 3"],
    ubication: "Pennsylvania, Paraguay",
    orchestra_type: "Orquesta de Cuerdas",
    donations_acount: "Meta.Cole@gmail.com",
    phone: "626-918-6270",
  },
];

export const Users = [
  {
    id: "1",
    cover: "http://placeimg.com/640/480/people",
    name: "Kellie Auer",
    email: "River.Koch@example.net",
    password: "wmAosoOrBGF2uKg",
    image:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/614.jpg",
    birthday: "Fri Apr 08 2022 12:07:36 GMT+0000 (Coordinated Universal Time)",
    city: "Port Wilmahaven",
    rol: "Teacher",
    about:
      "Quia explicabo porro similique. Sint voluptate dolorem nesciunt dignissimos inventore. A quia sed neque rerum. Vero deleniti sapiente quasi repellendus assumenda ea dolor optio. Repellendus facere itaque voluptatem sit ipsam. Qui in necessitatibus consequuntur illum ex et voluptates.",
  },
  {
    id: "2",
    cover: "http://placeimg.com/640/480/animals",
    name: "Beverly Buckridge II",
    email: "Dale_Doyle18@gmail.com",
    password: "jRbVqZDJlewevyQ",
    image:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/37.jpg",
    birthday: "Sat Dec 25 2021 15:36:17 GMT+0000 (Coordinated Universal Time)",
    city: "Tinley Park",
    rol: "Director",
    about:
      "Quae ratione quia omnis quaerat ab. Eos ut nobis qui molestiae nihil voluptatem. Ullam voluptas cum nemo dolorem voluptatem omnis pariatur. Iusto veniam quos reprehenderit est voluptatibus voluptatem. Ipsam sint eius. Aliquam deserunt a fugit sequi est iure est.",
  },
  {
    id: "3",
    cover: "http://placeimg.com/640/480/city",
    name: "Latoya Klein",
    email: "Don8@hotmail.com",
    password: "hGOziQSu67eSoHQ",
    image:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/168.jpg",
    birthday: "Mon Jan 03 2022 01:24:08 GMT+0000 (Coordinated Universal Time)",
    city: "East Romainetown",
    rol: "Student",
    about:
      "Eaque rerum deserunt est et cupiditate suscipit aut. Quae voluptates in est. Dolores recusandae ipsa voluptas fuga. Natus non sapiente quia fugiat. Eum excepturi voluptatem libero dolorem molestiae. Explicabo consequatur incidunt.",
  },
  {
    id: "4",
    cover: "http://placeimg.com/640/480/nature",
    name: "Cody Roberts",
    email: "Nicola_Franecki@gmail.com",
    password: "2sJ_KRysfWFszpW",
    image:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/465.jpg",
    birthday: "Fri Dec 17 2021 12:24:29 GMT+0000 (Coordinated Universal Time)",
    city: "Remingtonport",
    rol: "Student",
    about:
      "Esse voluptatum qui architecto ex facere. Sed qui animi quibusdam amet. A sed id consequatur totam mollitia laborum et ducimus. Quisquam aut quo delectus tempore quo sit labore. Dicta corporis blanditiis quam et est quas cupiditate ut.",
  },
];

export const Posts = [
  {
    title: "Eos magni ipsum.",
    content:
      "Blanditiis harum quidem tenetur voluptatem. Non dolorum vel repellat recusandae sunt ut. Enim deserunt et dolor et eveniet ipsam sed. Iste in ducimus consequatur id quis soluta. Excepturi tenetur quo dolor. Illum ut eum.",
    media: "http://placeimg.com/640/480/business",
    views: 91625,
    event_location: "Vero - Architecto - Asperiores",
    event_day: "Dec 03 2022",
    event_hour: "20:50",
    file: "card_keyboard_enterprise.pdf",
  },
  {
    title: "Laborum cum ea aliquid.",
    content:
      "Repudiandae illo accusantium sequi illum amet suscipit quia est voluptates. Non pariatur possimus in sapiente facilis. Labore ipsam qui dolore autem. Iste neque nihil dolorem non nisi quam est. Nisi rem quod. Placeat hic occaecati.",
    media: "http://placeimg.com/640/480/nature",
    views: 19671,
    event_location: "Modi - Vel - Impedit",
    event_day: "Oct 01 2023",
    event_hour: "17:45",
    file: "generic.mp4",
  },
  {
    title: "Unde beatae nostrum accusantium voluptatem aspernatur.",
    content:
      "Error est dignissimos soluta facere nihil inventore molestiae rem. At soluta unde veniam ut iure doloremque temporibus beatae dolorem. Quasi perferendis veritatis velit asperiores. Eaque molestiae quo voluptates provident accusamus numquam voluptate.",
    media: "http://placeimg.com/640/480/cats",
    views: 24075,
    event_location: "Dolorum - Quaerat - Sint",
    event_day: "Nov 21 2022",
    event_hour: "11:34",
    file: "malawi_incredible.csv",
  },
  {
    title:
      "Dolor iusto numquam perferendis cupiditate et aut cupiditate ratione deserunt.",
    content:
      "Quae suscipit in voluptas distinctio molestiae cumque dolore. Est dolores autem quisquam doloribus molestiae. Hic tenetur et dolore quia. Aliquam eos minima corrupti labore vel sit ea praesentium nemo. Doloremque odio consequatur libero ipsam sit accusamus atque voluptatem ut.",
    media: "http://placeimg.com/640/480/fashion",
    views: 79025,
    event_location: "Dolorum - Quaerat - Sint",
    event_day: "Nov 21 2022",
    event_hour: "11:34",
    file: "com.wav",
  },
];

export const Comments = [
  {
    user: "Sydnee.Torphy",
    content:
      "Aut eveniet ut ut enim et aspernatur consequatur natus. Molestiae est libero suscipit. Quae sed debitis amet iusto numquam atque.",
  },
  {
    user: "Mariana.Veum20",
    content:
      "Inventore dolor omnis exercitationem dolores ratione. Tempore quod perferendis sed reiciendis fugit autem. Assumenda ducimus odit et. Praesentium occaecati asperiores corporis. Blanditiis quis natus commodi.",
  },
  {
    user: "General76",
    content:
      "Optio ut ut quas ut nihil quo vel. Aut dolorum a amet perferendis aperiam nam enim tenetur quia. Est cupiditate dolores rerum voluptates atque ab dicta. Deserunt sapiente et laborum commodi natus aut dolor fugit eius. Ut aliquam qui beatae. Consequatur voluptatem dolor distinctio.",
  },
  {
    user: "Rowland26",
    content:
      "Sint est exercitationem dolorem consequuntur occaecati sunt provident nobis numquam. Alias et nostrum facere soluta numquam quidem corrupti beatae. Rerum porro velit quibusdam nemo id at et ipsam. Ullam id suscipit ut et id quasi deleniti maxime.",
  },
];

export const Campaigns = [
  {
    title: "Impedit ea ea.",
    description:
      "Quasi molestias autem incidunt at ratione aut explicabo enim quia. Sapiente cupiditate aut et. Id veniam autem dolor omnis. Sit sunt reprehenderit sed.",
    media:
      "https://res.cloudinary.com/dzup1ckpy/image/upload/v1668015686/imagen-post-metodologia-2-1_sazsnh.jpg",
    goal_amount: 23980,
    start_date: "Oct 09 2022",
    end_date: "Jul 13 2023",
    amount_raised: 642.17,
  },
  {
    title: "Excepturi soluta magni mollitia non nulla deserunt.",
    description:
      "Ad architecto odit voluptatem et beatae ullam ratione error dolor. Odit voluptas commodi voluptas occaecati. Dignissimos tempore occaecati dolor rerum. Dolore et esse aut non et dolores consequatur. Quis officiis minima deserunt qui sint voluptas vero.",
    media:
      "https://res.cloudinary.com/dzup1ckpy/image/upload/v1668015871/descarga_csz0n0.jpg",
    goal_amount: 38550,
    start_date: "Feb 08 2022",
    end_date: "Jun 16 2023",
    amount_raised: 741.78,
  },
  {
    title:
      "Accusantium harum iste consequatur suscipit harum officia minima aut.",
    description:
      "Veniam assumenda debitis. Quis perspiciatis ea. Modi recusandae quaerat quis quo eum consectetur est ut commodi. Sit et incidunt libero ut ipsam architecto eligendi eum. Est id sed assumenda est. Corrupti ducimus sequi possimus quia alias tempora aliquam nemo sapiente.",
    media:
      "https://res.cloudinary.com/dzup1ckpy/image/upload/v1668015883/destacada-3_rylbg3.webp",
    goal_amount: 21280,
    start_date: "Jul 19 2022",
    end_date: "Jan 12 2023",
    amount_raised: 740.12,
  },
  {
    title: "Nostrum ut unde explicabo quia.",
    description:
      "Itaque aperiam non. Sed dolor harum. Aspernatur reiciendis iste occaecati doloribus fugiat facilis dolorem laudantium. Repudiandae est ut non. Quo quam distinctio voluptas optio ea consequuntur dolores et aut. At recusandae itaque sit est dicta et ullam.",
    media:
      "https://res.cloudinary.com/dzup1ckpy/image/upload/v1668015863/pbn11122019-orchestra1_n4vhj4.jpg",
    goal_amount: 91590,
    start_date: "Mar 28 2022",
    end_date: "Apr 20 2023",
    amount_raised: 500.08,
  },
];

export const Developers = [
  {
    pic:"https://res.cloudinary.com/dzup1ckpy/image/upload/v1669867934/WhatsApp_Image_2022-12-01_at_12.11.18_AM_efep7g.jpg",
    name: "Hector Ivan Garcia",
    team: "Backend",
    description: "Soy de la ciudad de Bogotá/Colombia, un apasionado por la tecnologia y el desarrollo, este proyecto fue una de las mejores experiencias que he tenido conoci gente muy buena, trabajadora y divertida y mis metas es seguir creciendo como desarrollador y como persona.",
    GitUrl: "https://github.com/Dargnel",
    LinkUrl: "https://www.linkedin.com/in/ivan-garcia-5ab79b24a/",
  },
  {
    pic:"https://res.cloudinary.com/dzup1ckpy/image/upload/v1669868533/WhatsApp_Image_2022-12-01_at_12.21.55_AM_whv4yi.jpg",
    name: "Mercedes Ruiz",
    team: "Backend/Frontend",
    description: "Soy Mercedes Ruiz, tengo 29 años y soy de Argentina. Me adentre en el mundo de la programacion a principios del 2022 y a mediados de año entre a Henry. Para significó un enorme crecimiento tanto personal como academico, ya que pude superar muchas barreras que veia muy lejanas. Este proyecto lo afronte con muchisima alegria y ambición, ya que venimos compartiendo con este grupo desde principios del bootcamp y estan constantemente apoyandome en lo que necesite y tambien abiertos a aportes del resto de los compañeros.Termino este proyecto y esta etapa con un monton de aprendizajes y superacion, y solo tengo deseos de ir por mas y llegar a ser unabuena profesional. Me siento muy agradecida de haber formado parte.",
    GitUrl: "https://github.com/MECHIRUIZ",
    LinkUrl: "https://www.linkedin.com/in/mercedes-ruiz-4bba7320b/",
  },
  {
    pic:"https://res.cloudinary.com/dzup1ckpy/image/upload/v1669869108/jartisick_1666020188005136_evqhi4.jpg",
    name: "Jose Angel Rodriguez",
    team: "Frontend",
    description: "Mi nombre es José Angel Rodríguez, tengo 25 años y soy de Venezuela. Empecé este viaje hace aproximadamente 6 meses, este proyecto para mí fue una experiencia única, me tocó un tema excelente y un grupo extraordinario, lleno de gente linda y dispuestas a aportar en lo que sea que ocurra. Me siento muy afortunado de haber formado parte de este grupo",
    GitUrl: "https://github.com/jartisick",
    LinkUrl: "https://www.linkedin.com/in/jos%C3%A9-%C3%A1ngel-rodr%C3%ADguez-367b2817a/",
  },
  {
    pic:"https://res.cloudinary.com/dzup1ckpy/image/upload/v1669868767/WhatsApp_Image_2022-12-01_at_12.24.33_AM_kcv8n5.jpg",
    name: "Francisco Giudicatti",
    team: "Backend/Frontend",
    description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",
    GitUrl: "https://github.com/franciscoo11",
    LinkUrl: "linkedin.com/in/francisco-giudicatti/",
  },
  {
    pic:"https://res.cloudinary.com/dzup1ckpy/image/upload/v1669868110/WhatsApp_Image_2022-12-01_at_12.14.35_AM_zufien.jpg ",
    name: "Oscar Angel Cu",
    team: "Frontend",
    description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",
    GitUrl: "https://github.com/oscaracu",
    LinkUrl: "https://www.linkedin.com/in/oscaracu/",
  },
  {
    pic:"https://res.cloudinary.com/dzup1ckpy/image/upload/v1669868804/WhatsApp_Image_2022-12-01_at_12.24.33_AM_2_ixfqay.jpg",
    name: "Javier Gonzales",
    team: "Backend",
    description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",
    GitUrl: "https://github.com/hanseyll",
    LinkUrl: "https://www.linkedin.com/in/javier-jos%C3%A9-gonzalez-pacheco-791721244/",
  },
  {
    pic:"https://res.cloudinary.com/dzup1ckpy/image/upload/v1669868113/WhatsApp_Image_2022-12-01_at_12.14.51_AM_say58c.jpg",
    name: "Camila Soledad Panella",
    team: "Backend",
    description: "Mi nombre es Camila Panella. Soy de la ciudad de La Plata, Argentina. Nueva en el mundo de la programacion, este proyecto me ayudo a entender y aprender varios conceptos nuevos, gracias a mis compañeros. Me gustaria especializarme en el desarrollo back-end. Mi meta en adelante es seguir avanzando e incorporando conocimientos.",
    GitUrl: "https://github.com/camilapanella",
    LinkUrl: "https://www.linkedin.com/in/camila-panella-8150aa251/",
  },
  {
    pic:"https://res.cloudinary.com/dzup1ckpy/image/upload/v1669868784/WhatsApp_Image_2022-12-01_at_12.24.33_AM_1_xss40s.jpg",
    name: "Juan Carlos Lange",
    team: "Frontend",
    description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",
    GitUrl: "",
    LinkUrl: "https://www.linkedin.com/in/juancarloslange/",
  },
]
